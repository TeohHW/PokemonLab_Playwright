...
| --------------------- | --------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shared app shell      | `tests/specs/application-shell.spec.ts` | 9            | Fresh bare-route station entry, Resume-only saved context and parameter restoration, hash-route continuity, browser Back/Forward, invalid Continue-state handling, far-right Resume alignment, absence of empty/favorites panels, SVG favicon delivery, station-menu navigation, scroll lock, initial-action focus, active-station styling, focus recovery and trapping, focus restoration, and persisted reduced-motion preference                                                                                                                                                    |
| Pokemon Pokedex       | `tests/specs/pokedex.spec.ts`           | 50           | Fresh home entry without the saved Pokedex or Pokemon, direct-route and reload restoration, paged navigation, typeahead search and inline clearing, profiles, previous/next boundaries, comparison, ability/move dialogs, generation-grouped move-version selection and equivalence guidance, collapsible evolution/card/sprite sections, game Pokedex filters including Legends Z-A, sorting, random Pokemon, image fallback, responsive detail sections, and rapid-action reliability                                                                                                |
| Pokemon TCG Simulator | `tests/specs/tcg-simulator.spec.ts`     | 56           | Fresh Base expansion home entry, data-load Retry, expansion routes and reload restoration, paginated expansion/Pokemon search, collapsible expansion browsing, desktop Series buttons and tablet/mobile dropdown behavior, newest-first sorting with normally interleaved release types, eight-result desktop/mobile layouts, pack preparation, Skip/Reveal All, keyboard card reveals, Binder All/Owned/Missing filters, progress and clearing, rarity sorting, artwork candidates, Special / Limited expansions, durable collection persistence, and persistent mobile pack actions  |
...
| Pokemon Team Planner  | `tests/specs/team-planner.spec.ts`      | 54           | Fresh empty home entry, Resume-only saved-workspace restoration, durable twelve-entry named-team library, normally scrolling Build/Analysis controls, separate workspaces, independently expanding member options with non-stretching neighboring cards, build-control spacing, browsing/search/sorting, game Pokedex filters, manual/meta/random/champion teams, battle-format legality, forms, abilities, natures, moves, assistant scoring/recommendations, mobile slot selection, six-slot desktop preservation, navigation, and reliability checks                                |
...
| TrainerDex            | `tests/specs/trainerdex.spec.ts`        | 32           | Fresh default trainer/game/stage home entry, direct trainer routes and reload restoration, invalid-route fallback, search, previous/next boundaries, region and game-version splits including Janine and Legends Z-A, distinct rematches across generations, grouped starter-dependent teams, removal of duplicate battle stages and source panels, explicit unverified-move states, current base-stat headings, wrapped mobile stage controls, pointer/keyboard TCG-card dialogs, responsive sections and filters, failed artwork removal, delayed data, and rapid-action reliability |

Normal tests run across configured Chromium, desktop Firefox, desktop WebKit, and mobile Chrome projects. Live PokeAPI suites run in Chromium. The only conditional skip is the station-menu pointer-focus assertion on WebKit, whose click behavior does not establish the opener as the active element; that assertion continues to run in Chromium, Firefox, and mobile Chrome.

## Coverage Themes

- Station entry and initial UI state from the shared home screen
- Shared hash routing, direct links, browser history, fresh station-card entry, Resume-only saved context, and local preferences
...
- State changes such as score updates, finite-session boundaries, binder progress and filters, team count, and leaderboard entries
- Team Planner save-state coverage, including an empty fresh workspace, Resume-only workspace restoration, durable named-team library limits, save/load/update/delete, unsaved teams, and malformed local storage
- Responsive layout checks at representative web and mobile widths, including sticky detail navigation, internal carousels, persistent action controls, one-member team selection, desktop-content preservation, and page-overflow protection across all six stations
- Reference-only TCG expansions, including disabled pack opening and complete-by-default binders
...
- No application behavior is hidden behind `test.fixme`; supported behaviors are asserted directly. Only the documented WebKit pointer-focus case is conditionally skipped.
