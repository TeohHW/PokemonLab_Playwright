...
| --------------------- | --------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pokemon Pokedex       | `tests/specs/pokedex.spec.ts`           | 45           | Initial load, paged Pokemon navigation, search, profiles, ability and move details, evolution navigation, sprite sets, game Pokedex filters including Legends Z-A, sorting, random Pokemon, image fallback, responsive web/mobile layouts, and rapid action reliability                                                                      |
| Pokemon TCG Simulator | `tests/specs/tcg-simulator.spec.ts`     | 48           | Session startup, pack opening, pack modal behavior, binder progress, collection clearing, bidirectional binder rarity sorting, unowned-card artwork display, set selection, Special / Limited reference sets, set/Pokemon combined search, and responsive web/mobile layouts                                                                 |
| Who's That Pokemon    | `tests/specs/whos-that-pokemon.spec.ts` | 36           | Setup, trainer validation, region selection, gameplay, guessing flow, help choices, leaderboard handling, responsive web/mobile layouts, navigation, local storage reliability, and network/image failure behavior                                                                                                                           |
| Pokemon Team Planner  | `tests/specs/team-planner.spec.ts`      | 48           | Station startup, browsing/search/sorting, game Pokedex filters, manual/meta/random/champion team building, saved-team persistence and reload restoration, battle-format legality, duplicate handling, forms, abilities, natures, moves, assistant scoring/recommendations, responsive web/mobile layouts, navigation, and reliability checks |
| Pokemon Quiz          | `tests/specs/pokemon-quiz.spec.ts`      | 29           | Station startup, quiz pool/category selection, playable questions, scoring panel updates, reset behavior, active-quiz leave confirmation, responsive web/mobile layouts, navigation, and rapid-start reliability                                                                                                                             |
| TrainerDex            | `tests/specs/trainerdex.spec.ts`        | 22           | Station startup, trainer search, region, game-version and battle-stage selection including Legends Z-A, trainer detail panels, team Pokemon TCG-card modal behavior, responsive web/mobile layouts, image fallback, delayed data, and rapid action reliability                                                                               |
...
- Team Planner save-state coverage, including explicit saves, restored settings and build choices after reload, unsaved teams, and malformed local storage
